...
let cfg_returns ({input;blocks;_} : cfg) : (loc option * _ basicBlock BlockMap.t) E.t = 
  let rec aux lbl blocks = 
    ...
    | Some bb -> 
      ...
      | Some Break -> E.throw Logging.(make_msg bb.location "there should be no break at this point") >>= fun () -> aux input blocks'
...
let check_unreachable (proto : method_sig) (_,cfg : mir_function) : unit E.t = 
  ...
  if Option.is_some ret && proto.rtype <> None then 
    E.throw Logging.(make_msg (Option.get ret) @@ Printf.sprintf "%s doesn't always return !" proto.name)
  else
    let () = BlockMap.iter (fun lbl {location=l;_} ->  Logs.debug (fun m -> m "unreachable block %i, line %i" lbl (fst l).pos_lnum)) unreachable_blocks in
    ...
    | Some (_,bb) -> 
      let loc = match List.nth_opt bb.assignments 0 with
      | Some v -> v.location
      | None ->  bb.location in
      E.log Logging.(make_msg loc"unreachable code")
    | None -> E.pure () 
  *)
...
let check_returns  (proto : method_sig) (decls,cfg : mir_function) : mir_function E.t = 
  ...
  let check block = 
    ...
    | None,Some _  -> 
      E.throw Logging.(make_msg proto.pos
    ...
    let+ methods = ListM.map (
      fun f -> 
        let+ m_body = match f.m_body with
                    ...
                    | Right b -> 
                      (* check_unreachable f.m_proto b >>= fun () -> not correct *)
