...
type body = (HirUtils.statement,(HirUtils.statement,HirUtils.expression) SailParser.AstParser.process_body) SailModule.methods_processes
...
let find_process_source (name: l_str) (import : l_str option) procs : 'a process_defn option M.t =
  let* _,env = M.get in 
  let* origin,_ = HirUtils.find_symbol_source ~filt:[P()] name import env |> M.from_error in
  let+ procs = 
    if origin.value = HirUtils.D.get_name env then return procs
    else
      let find_import = List.find_opt (fun i -> i.mname = origin.value) (HirUtils.D.get_imports env) in
  ...
  in
  List.find_opt (fun (p:_ process_defn) -> p.p_name = name.value) procs
