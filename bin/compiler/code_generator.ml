...
let methodToIR (method_proto : llvalue) (m_args : (llbuilder->llvalue) array) (p_body : Ast.statement) (llm:llmodule) (context:llcontext) : unit =
  let env : SailEnv.t = SailEnv.empty () in 
  let builder = builder context in
  let bb = append_block context "" method_proto in
...
let processToIR (method_proto : llvalue) (p_body : Ast.statement) (llm:llmodule) (context:llcontext) : unit =
    let env : SailEnv.t  = SailEnv.empty () in 
    let builder = builder context in
    let bb = append_block context "" method_proto in
    position_at_end bb builder;
...
let methodArgs (args: (string * Common.sailtype) list ) (context:llcontext) : (llbuilder -> llvalue) array = 
    let llvalue_list = List.map (
      fun (name, t) -> 
        let  ty = getLLVMType t context in
        build_alloca ty name
    ) args in
    Array.of_list llvalue_list


let parse_method (s : Ast.statement Common.method_defn) (llm:llmodule) (context:llcontext) : unit =
  let method_rt = match s.m_rtype with
  | Some t-> getLLVMType t
  | None -> void_type
  in
  let method_t = 
    let args_type = List.map (fun (_,arg) -> getLLVMType arg context) s.m_params |> Array.of_list in
    function_type (method_rt context) args_type 
  in
  ...
  in 
  let args = methodArgs s.m_params context in
    methodToIR method_proto args s.m_body llm context |> ignore;
...
let parse_process (s: Ast.statement Common.process_defn) (llm:llmodule) (context:llcontext) : unit =
    let args : lltype Array.t = [||] in
    let name = if String.equal "Main" s.p_name then "main" else s.p_name in
    let process_t = function_type (void_type context) args in
    ...
    in 
    processToIR process_proto s.p_body llm context |> ignore;
